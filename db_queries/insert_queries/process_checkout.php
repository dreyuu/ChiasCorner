...
try {
    ...
    //     // Check if any batches remain for the ingredient
    //     $stmt = $connect->prepare("SELECT COUNT(*) FROM stock_batches WHERE ingredient_id = :ingredient_id");
    //     $stmt->execute([':ingredient_id' => $ingredientId]);
    //     $batchCount = $stmt->fetchColumn();

    //     if ($batchCount == 0) {
    //         // No batches left, safe to remove from inventory
    ...
    $stmt = $connect->prepare("SELECT total_price FROM orders WHERE order_id = :order_id");
